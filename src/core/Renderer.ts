...
export interface RendererInterface extends RendererSetupInterface {
  ...
  addListener<T extends keyof EventCallbackMap>(
    t: T,
    cb: EventCallbackMap[T]
  ): void;
  removeListener<T extends keyof EventCallbackMap>(
    t: T,
    cb: EventCallbackMap[T]
  ): void;
}
...
export default defineComponent({
  ...
  mounted() {
    ...
    if (this.three.init()) {
      ...
      this.renderFn = this.three.composer
        ? this.three.renderC
        : this.three.render;
    ...
    this.onMounted?.(this as RendererInterface);
    this.mountedCallbacks.forEach((e) =>
      e({ type: "mounted", renderer: this })
    );
  },
  beforeUnmount() {
    // this.canvas?.remove()
  ...
  methods: {
    addListener(type: EventType, cb: { (e?: any): void }) {
    ...
    render(time: number) {
      this.onBeforeRender?.({ type: "beforerender", renderer: this, time });
      this.beforeRenderCallbacks.forEach((e) =>
        e({ type: "beforerender", renderer: this, time })
      );
      // this.onFrame?.(cbParams)
      this.renderFn({ renderer: this, time });
      this.onAfterRender?.({ type: "afterrender", renderer: this, time });
      this.afterRenderCallbacks.forEach((e) =>
        e({ type: "afterrender", renderer: this, time })
      );
    },
