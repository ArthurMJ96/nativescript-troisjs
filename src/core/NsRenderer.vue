...
import { ref, shallowRef, computed, nextTick, PropType } from "vue";
...
const props = defineProps({
  ...
  pointer: {
    type: [Boolean, Object] as PropType<boolean | PointerPublicConfigInterface>,
    default: false,
  },
  resize: {
...
const emit = defineEmits<{
  canvasReady: [canvas: TCanvas];
  rendererReady: [renderer: RendererInterface];
}>();
...
<template>
  <ContentView ref="el" @loaded="onLoaded">
    <Canvas v-if="appReady" @ready="onCanvasReady" />
    <Renderer
      v-if="canvas && appReady"
      ref="renderer"
      v-bind="props"
      :outer-canvas="canvas"
      :height="sizes.height"
      :width="sizes.width"
      @ready="emit('rendererReady', $event)">
      <slot />
    </Renderer>
  </ContentView>
</template>
